...
struct DispatchFactory {
    ...
    std::map<std::string, Callable> registry;

    void dispatch(const std::string& identifier, T& arg, YAML::Node& cfg) const {
    ...
    bool register_method(const std::string& identifier, const Callable& constructor) {
        auto it = registry.find(identifier);
        if (it != registry.end()) {
            return false;
        }
...
#define MOSSCAP_PROB_FN_SIGNATURE(PROB_NAME) void MOSSCAP_PROB_FN_NAME(PROB_NAME)(Simulation& sim, YAML::Node& config)
