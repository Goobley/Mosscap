...
MOSSCAP_NEW_PROBLEM(coronal_rain_drop_2d) {
    ...
    const fp_t rho_0 = FP(5e-12);
    const fp_t P_0 = FP(0.165);
    const fp_t T_0 = FP(2e6);
    const fp_t g = get_or<fp_t>(config, "sources.gravity.y", FP(-274.0));
    const fp_t mean_mass = FP(1.0);
    const fp_t H = k_B * T_0 / (mean_mass * h_mass * -g);
    // const fp_t rho_b0 = FP(10.0) * rho_0;
    const fp_t rho_b0 = FP(5e-10);

    constexpr fp_t x0 = FP(0.0);
    constexpr fp_t z0 = FP(50e6);
    constexpr fp_t delta = FP(0.5e6);
    ...
    for (int i = sz.ng + 1; i < sz.yc; ++i) {
        ...
        const f64 rho_half = FP(0.5) * P_half / (k_B * T_half) * (mean_mass * h_mass);

        const f64 dP_dz_mid = rho_half * g;
        pressure(i) = pressure(i - 1) + dP_dz_mid * dz;
        rho(i) = FP(0.5) * pressure(i) / (k_B * T_0) * (mean_mass * h_mass);
